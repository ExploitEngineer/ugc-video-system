...
export function buildInterpretStylePrompt(
  ...
  const system = [
    ...
    "OVERRIDES all content inference (do NOT drift to a cinematic catch-all). Map:",
    "  'UGC' / 'user-generated' / 'testimonial' / 'review' / 'talking to camera' ->",
    "    testimonial;",
    "  'unboxing' -> unboxing;  'explainer' / 'how it works' / 'the science' -> explainer;",
    "  'promo' / 'sale' / 'discount' / 'offer' / 'coupon' / '% off' -> promo-offer;",
    "  'announcement' / 'launch' / 'introducing' / 'now available' -> announcement;",
    "  'manifesto' / text-only 'brand film' -> brand-awareness;",
    "  'demo' / 'in action' / 'watch it work' -> product-demo;",
    "  'showcase' / 'hero shots' / 'show off' -> product-showcase;",
    "  'comparison' / ' vs ' / 'us vs them' -> comparison;",
    "  'before and after' / 'transformation' -> before-after;",
    "  'founder' / 'why I built' / 'why we built' -> founder-pov;",
    "  'spokesperson' / 'VSL' / 'avatar host' / 'presenter pitch' -> spokesperson;",
    "  'lifestyle' -> lifestyle;  'social proof' / 'reviews wall' / 'star ratings' -> social-proof;",
    "  'problem-agitate-solve' / 'PAS' / 'tired of' / 'sick of' -> problem-agitate-solve;",
    "  'brand story' / 'origin story' -> brand-story;  'inspirational' / 'mood piece' -> inspirational.",
    "Otherwise, pick the single best `id` from the AD TYPE MENU using the cues. Use",
    "the ground-truth hasProduct/hasPerson flags as strong evidence. If the prompt is",
    "vague or empty AND no type is named: if hasProduct is true choose",
    "product-showcase, else choose brand-awareness (NEVER default to brand-story).",
    "spokesperson = ONE presenter addressing the viewer directly. If the script has",
    "TWO OR MORE named characters in dialogue WITH EACH OTHER (a scripted scene, a",
    "sketch, a two-hander), it is NOT spokesperson and NOT testimonial (one",
    "unscripted reviewer) — classify by the DOMINANT look: handheld / candid →",
    "a ugc_authentic type (lifestyle or problem-agitate-solve); cinematic →",
    "brand-story or inspirational.",
    "",
