...
export function buildInterpretStylePrompt(
  ...
  const system = [
    ...
    '- Each hook is {"id": <hook id from the HOOK MENU above>, "role": "visual_lead" | "overlay"}.',
    '- If 2 hooks: EXACTLY ONE has role "visual_lead" and the other "overlay". Never two visual_leads.',
    "- Pick the hook whose device best matches THIS prompt's specific angle and",
    "  emotional entry point; use the type's default hooks only as a fallback when",
    "  the prompt gives no strong signal. Do NOT reflexively pick the default.",
    "- Asset guardrail (use the ground-truth flags): a person-gated hook",
    "  (confession) needs a real on-screen person — if hasPerson is false do NOT",
    "  pick it; substitute the type's first compatible default.",
    "",
    "## TASK 4 — assetIntent",
