...
export function buildTemplateBlueprintPrompt(
  ...
  const system = [
    ...
    visualIntro,
    "",
    "You produce a STRUCTURAL blueprint: the template's LOOK, PACING, TIMING, CAMERA",
    "and TEXT-OWNERSHIP. You do NOT choose the ad's SUBJECT — the product and its",
    "scenes are authored separately, downstream, from the user's OWN uploaded product.",
    "So every field you write is SUBJECT-AGNOSTIC: describe the ROLE and STRUCTURE of",
    "each slot, never a concrete product, object or brand. The frames show the",
    "template's DEMO placeholder content; read its STYLE and STRUCTURE only, and NEVER",
    "carry the demo's subject (whatever is held, worn, unboxed, poured or used) into",
    "any field. Your output must read the SAME whether the demo showed a bottle, a",
    "shoe or a pair of glasses.",
    "",
    "Rules:",
    ...
    hasText
      ...
      : "",
    hasImages
      ? "- IMAGE slots: set `fill` true when the slot should hold a photographic still of\n  this ad's product/world, and write `imageSubject` as a SUBJECT-AGNOSTIC shot ROLE\n  — e.g. \"hero product shot\", \"product-in-use lifestyle shot\", \"close detail shot\" —\n  NEVER a specific product noun; the real product is composited downstream from the\n  user's upload. Set `fill` false when it is clearly not a photo, and omit\n  `imageSubject`. When unsure, prefer false — the template's own artwork is the safe\n  fallback."
      : "",
    `- VIDEO slots: the whole ad is ONE continuous ~${input.clipSeconds}s take. For`,
    "  EACH video slot write `videoScene` as the STRUCTURAL beat — its shot ROLE,",
    "  framing and the KIND of action, with NO product noun (e.g. \"opening hero reveal,",
    "  subject presented to camera\", \"close detail beat\", \"in-use lifestyle beat\",",
    "  \"confident sign-off to camera\"); `cameraAction` (one move or a hold); and",
    "  `onScreenMoment` (what KIND of thing visibly happens in those seconds, subject-",
    "  agnostic) — as consecutive beats of that single take. Keep the SAME place, person",
    "  and look across every beat; only framing and the small moment advance. They are",
    "  slices of one shot, not separate scenes.",
    "- `audio`: `voiceover` = what the VO should CONVEY structurally (hook → benefit →",
    "  CTA), intent only and with NO product noun; `tone` = the voice character",
    "  (age/gender/energy). Keep both short.",
    "",
    "Return STRICT JSON only (no prose, no markdown fences):",
    "{",
    '  "conceptSummary": "<one sentence: the STRUCTURAL through-line/arc all slots share, e.g. \'reveal → detail → in-use → sign-off\' — no product noun>",',
    ...
    "",
    "HARD RULE (the most important one): do NOT name, describe or imply ANY specific",
    "product, object or brand anywhere in your output. Whatever the demo frames show",
    "being held, worn, unboxed or used is PLACEHOLDER content to IGNORE. The user's",
    "real product is attached downstream; your blueprint only says HOW the ad looks,",
    "moves and is paced — never WHAT it sells.",
  ]
    .filter(Boolean)
    .join("\n");
