...
//   SPEC   = explicitly named in the seam map. Move verbatim.
//   INFER  = a likely additional inline ternary; wire it to the real ternary
//            during the verbatim-move pass (Chunk F), OR return [] if no such
//            ternary exists. A method with no source is a harmless no-op in the
//            array spread.
//
...
export interface FragmentSet {
  // ---- storyboard seams (image/storyboard/prompt.ts) ----
  ...
  /** INFER ← panel-caption ternary (casual UGC captions vs minimal cinematic). LOOK-driven. */
  storyboardCaptionStyle(ctx: FragmentCtx): string[];
  /** INFER ← transcript-line ternary (first-person spoken review vs VO narration). TYPE-driven. */
  storyboardTranscriptStyle(ctx: FragmentCtx): string[];
  /** INFER ← per-panel camera/shot ternary (handheld vs composed). LOOK-driven. */
  ...
  /** INFER ← shot-rhythm/pacing ternary (quick handheld cuts vs smooth cinematic). LOOK-driven. */
