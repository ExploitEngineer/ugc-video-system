...
export function setupActions(cesdk: CreativeEditorSDK): void {
  ...
  // NOTE: `exportDesign` is intentionally NOT registered here. The "Export Video"
  // button runs the `exportDesign` action, and `registerActions` (../actions.ts)
  // overrides it to UPLOAD the exported MP4 (+ scene) back to the run instead of
  // downloading a file. That override runs after this plugin, so it wins.
  void cesdk;
  // #endregion
