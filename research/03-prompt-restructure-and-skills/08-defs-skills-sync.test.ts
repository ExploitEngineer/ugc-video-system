// apps/api/src/agents/ad-types/__tests__/defs-skills-sync.test.ts
//
...
//   4. each registered def implements every FragmentSet seam
//
// Uses vitest; swap describe/it/expect for the repo's runner if different.
...
import { FRAGMENT_SEAMS, type AdTypeDef, type FragmentCtx } from "../types";

const HERE = __dirname;
const DEFS_DIR = path.resolve(HERE, "../defs");
// Adjust REPO_ROOT depth to wherever apps/api sits relative to .claude.
...
// NOTE: def filenames are kebab-case ids; the exported symbol may be camelCase
// (e.g. brand-story.ts exports `brandStory`). We assert via the REGISTRY (which
// imports every def) rather than re-deriving the export name from the filename.
...
describe("ad-types defs ⇄ skills are in sync", () => {
  ...
  it("every registered def implements all fragment seams", () => {
    const ctx = stubCtx();
    for (const def of Object.values(REGISTRY) as AdTypeDef[]) {
      for (const seam of FRAGMENT_SEAMS) {
        ...
        // must return an array (RAW string[]), never a joined string
