...
- [x] Added `AssetCtx` + `willGenerateProduct`/`willGeneratePerson`/`hasAnyReference` predicates to `plan.ts`; `assetCtxFor(run, uploads)` in the orchestrator reads the registry `assetPolicy` (read-only) + uploads. **Note:** the reference phase is PARALLEL in the orchestrator (`runReferencePhase`), not sequenced by `nextStep`, so the product/person skip is implemented there (semantically equal to the SPEC's sequential model). Typecheck.
- [x] `firstStep(asset?)`: product not generated → start at `person_sheet`; neither generated → `storyboard`. Unit.
- [x] Product skip — `runReferencePhase` only generates the product sheet when a product was uploaded (product is never synthesized). Unit (`willGenerateProduct`). (With the current mandatory product upload this never triggers; non-breaking.)
- [x] Person: required+no-upload still synthesizes; not-required+no-upload skips (`runReferencePhase` gates the person branch on `willGeneratePerson`). Unit. (Behavior change: a `brand-story` run with no person upload now SKIPS the person sheet instead of inventing one.)
- [x] `gateForNext(next, hasReference)` collapses the reference gate when no reference sheet exists; the orchestrator threads `referenceExists` from `runReferencePhase`. Unit.
- [x] `genStepForRevise(gate, target, duration, asset?)` targets `product_sheet` on a person-skipped reference-gate revise, else `person_sheet`. Unit.
- [x] DONE (code) — 66 ad-types/plan unit tests green; `pnpm typecheck` (all pkgs) green. Merged into `dev` locally. **User's live check (a person-skipped `brand-story` run; a neither-asset run becomes reachable once Chunk H + the dropdown allow it) is the final verification.**
...
- [x] Backend: `POST /runs` accepts optional `adType` (+`"auto"`); a non-`auto` pick stores it + sets `ad_type_source="user"` (else both left null → auto-detect). `createRunInputSchema.adType` (open string) in shared.
- [x] Exposed the ad-type menu via `GET /ad-types` (`adTypeMenuList()` — id + displayName + whenToUse + assetPolicy, registry-driven so it grows with H) + the shared `adTypeMenuItemSchema`/`AdTypeMenuItem`. Verified via menu smoke (2 types).
- [x] Frontend: `AdTypeSelect` dropdown in `OptionsMenu` (default "Auto-detect" + the fetched types via `fetchAdTypes()` + the `/api/ad-types` Next proxy), submits `adType`. Native select so it scales 2→16.
- [x] Lock honored end-to-end: a `"user"` pick → `adTypeSource="user"` → orchestrator's queued block keeps the locked `adType` (Chunk E) while the detector still fills adStyle + hooks; "auto" runs full detection. (User's live pick/auto check is the final confirmation.)
- [x] DONE (code) — `pnpm typecheck` (all pkgs) + `pnpm --filter web lint` green. Merged into `dev` locally.
...
- [x] Backend: `RunDetail` now carries the typed `hooks` (`hookSelectionSchema` resolved selection) + `adTypeSource` (already base) + server-mapped `adTypeDisplayName` + `lookFamily`; `lib/mappers.ts` validates the hooks jsonb and resolves the display name / look family via `getAdType`. (Hook attribution is surfaced from the run-level hooks on scene 1, since Chunk F splices the opening there — no per-Scene column needed.) Typecheck.
- [x] Frontend: registry `displayName` adType chip + "· you chose / · auto" badge from `adTypeSource` (replaces the 2-value chip) in `run-view.tsx`. The script panel's chip + spoken/voiceover label now derive from `adTypeDisplayName` / `lookFamily` (fixes the post-E mislabel).
- [x] Frontend: visual-lead (+ overlay) hook chips in the run header + a per-scene "Hook · …" badge on scene 1 in `script-panel.tsx`.
- [x] Frontend: selected options surfaced — the existing duration/aspect/mode chips, plus an "AI person" chip when a person sheet exists with no uploaded photo (synthesized-person note).
- [x] DONE (code) — `pnpm typecheck` (all pkgs) + `pnpm --filter web lint` green; 66 api tests still green. Merged into `dev` locally. (User's review of a completed run is the final visual check.)
...
- **Chunk K — run-view detected-options UX landed (`feat/run-view-detected-options`, merged into `dev`).** The run view now surfaces the detected/chosen ad type, the hooks (and which scene they drive), and the selected options. **Shared/API:** `RunDetail` gains the typed `hooks` (`hookSelectionSchema` = `{visualLead, overlay}` of `{id, role, openingDirective}`), the server-mapped `adTypeDisplayName` + `lookFamily` (from `getAdType`), and already carried `adTypeSource`; `lib/mappers.ts` validates the `hooks` jsonb at the wire boundary and resolves the display name / look family. **Web:** `run-view.tsx` replaces the 2-value adType chip with a registry `displayName` chip + a "· you chose / · auto" source badge, adds visual-lead (+ overlay) hook chips and an "AI person" chip (person sheet present, no upload). `script-panel.tsx` derives its chip + the Spoken/Voiceover line label from `adTypeDisplayName`/`lookFamily` (fixing the post-Chunk-E mislabel where a `testimonial` run read as "Inspirational"), and shows a per-scene "Hook · …" badge on scene 1 (where Chunk F splices the opening). **Verified:** `pnpm typecheck` (all pkgs) + `pnpm --filter web lint` green; 66 api unit tests green.
- **Chunk J — ad-type selection dropdown landed (`feat/ad-type-dropdown`, merged into `dev`).** The create form now offers an optional ad-type override (default **Auto-detect**); an explicit pick locks the type. **Backend:** `POST /runs` accepts an optional `adType` form field — a non-`"auto"` value stores it + sets `ad_type_source="user"` (the orchestrator's queued block already honors a `"user"` lock from Chunk E), while `"auto"`/omitted leaves both null for full detection. New `GET /ad-types` route returns `adTypeMenuList()` (id + displayName + whenToUse + assetPolicy, registry-driven so it auto-grows with Chunk H); shared `createRunInputSchema.adType` (open string) + `adTypeMenuItemSchema`/`AdTypeMenuItem`. **Frontend:** `fetchAdTypes()` (via a new `/api/ad-types` Next proxy) loads the menu; an `AdTypeSelect` native dropdown (default "Auto-detect" + the types) sits in the create form's `OptionsMenu` and submits `adType`. **Verified:** `pnpm typecheck` (all pkgs) + `pnpm --filter web lint` green; the menu endpoint returns the 2 registered types.
- **Chunk G — conditional product/person step-skipping by asset policy landed (`feat/conditional-asset-steps`, merged into `dev`).** The reference phase now generates only the sheets a run actually needs, driven by the registry `assetPolicy` for the resolved ad type. **`plan.ts`:** new `AssetCtx` + `willGenerateProduct` (product comes ONLY from an upload — never synthesized), `willGeneratePerson` (uploaded OR `person:"required"` → synthesized; optional + none → skip), `hasAnyReference`; `firstStep(asset?)` skips to `person_sheet`/`storyboard` when earlier sheets are skipped; `gateForNext(next, hasReference)` collapses the reference gate when neither sheet exists; `genStepForRevise(…, asset?)` targets `product_sheet` on a person-skipped reference-gate revise. **Orchestrator:** `runReferencePhase` computes the asset ctx from `getAdType(ctx.adType).assetPolicy` + uploads, gates the product-sheet task and the person branch accordingly, and returns `hasReference`; the driver threads `referenceExists` into `gateForNext` and `assetCtxFor(run, uploads)` into the revise targeting. The reference phase stays PARALLEL (the SPEC's sequential firstStep/nextStep model maps onto it). **Behavior:** non-breaking for `testimonial` (product uploaded + person synthesized) and `brand-story`-with-person; a `brand-story` run with NO person upload now SKIPS the person sheet (was: invent one) — the intended asset-policy behavior. A neither-asset run (no sheets, collapsed gate) becomes reachable once Chunk H + the dropdown allow product-less runs. **Verified:** 66 ad-types/plan unit tests green (15 new plan tests); `pnpm typecheck` (all pkgs) green.
